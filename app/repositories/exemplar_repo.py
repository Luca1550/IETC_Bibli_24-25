...
class ExemplarRepo():
    ...
    def __init__(self):
        ...
        """
        self.exemplar_json : list[Exemplar] = JsonStorage.load_all(self.PATH_EXEMPLAR_JSON)
    
    def _save_all(self):
        """
        Saves all exemplars to the JSON file.
        """
        JsonStorage.save_all(self.PATH_EXEMPLAR_JSON, self.exemplar_json)
    ...
    def add_exemplar(self, exemplar : Exemplar):
        ...
        if isinstance(exemplar, Exemplar):
            self.exemplar_json.append(exemplar)
            self._save_all()
            return True
        return False